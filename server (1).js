 ...
 */
// ══════════════════════════════════════════════════════════════════
// 📧 EMAIL MASKELEME — ön uca gönderilmeden önce e-postayı gizle
// Örnek: ozgur@gmail.com → oz****@gmail.com
...
function maskEmail(email) {
    ...
    const domain = email.slice(atIdx);           // @gmail.com
    // Göster: ilk 2 karakter (veya 1 karakter, local çok kısaysa)
    const showLen = Math.min(2, Math.max(1, Math.floor(local.length / 3)));
    const visible = local.slice(0, showLen);
...
function formatUserUrls(user) {
    ...
    // 🔒 E-posta daima maskeli gönderilir — ham e-posta asla ön uca çıkmaz
    if (u.email) u.email = maskEmail(u.email);
    return u;
}

// Kendi profilini getirirken kullanılacak — email maskeli ama kullanıcının kendi maili
function formatOwnUser(user) {
    if (!user) return user;
    const u = formatUserUrls(user);
    return u; // email zaten maskeli
}

// 🔒 LIKE injection önlemi — % ve _ karakterleri özel anlam taşır
// Escape edilmezse saldırgan tüm tabloyu taratabilir (performance DoS)
...
async function sendLoginNotificationEmail(userEmail, userName, req, resetToken = null) {
    const now = new Date();
    // 🔒 req.ip kullan (trust proxy tarafından doğrulanmış) — x-forwarded-for manipüle edilebilir
    ...
    try {
        ...
        return res.json({
            ...
            user: {
                ...
                username    : user.username,
                email       : maskEmail(user.email),   // 🔒 Maskeli
                profilePic  : picUrl,
                avatar_url  : picUrl,
                isVerified  : user.isVerified,
                hasFarmerBadge: user.hasFarmerBadge,
...
app.get('/api/users/search', authenticateToken, async (req, res) => {
    ...
    try {
        const { rows } = await pool.query(`
            ...
            FROM users u
            WHERE (LOWER(u.username) LIKE $1 ESCAPE '\\' OR LOWER(u.name) LIKE $1 ESCAPE '\\')
...
app.get('/api/auth/me', authenticateToken, async (req, res) => {
    try {
        ...
        if (!user) return res.status(404).json({ error: 'Kullanıcı bulunamadı' });
        res.json({ user: formatOwnUser({ ...user, profilePic: absoluteUrl(user.profilePic), coverPic: absoluteUrl(user.coverPic) }) });
    } catch (e) {
...
app.post('/api/auth/verify-otp', otpLimiter, validateAuthInput, async (req, res) => {
    ...
    if (tempToken && code) {
        ...
        try {
            ...
            if (userSafe.email) userSafe.email = maskEmail(userSafe.email);
    ...
    try {
        ...
        const users = await dbAll(
            `SELECT id, username, name, "profilePic", "isVerified", "hasFarmerBadge"
             FROM users
             WHERE "isActive" = TRUE AND (LOWER(username) LIKE $1 ESCAPE '\\' OR LOWER(name) LIKE $1 ESCAPE '\\')
...
app.get('/api/search/users', authenticateToken, async (req, res) => {
    try {
        ...
        const offset = (Math.max(parseInt(page)||1, 1) - 1) * Math.min(Math.max(parseInt(limit)||20,1), 200);
        const searchTerm = `%${escapeLike(q.toLowerCase().trim())}%`;
        ...
        const users = await dbAll(
            `SELECT id, username, name, "profilePic", "isVerified", "hasFarmerBadge", "userType",
             ...
             FROM users
             WHERE "isActive" = TRUE AND (LOWER(username) LIKE $2 ESCAPE '\\' OR LOWER(name) LIKE $2 ESCAPE '\\')
    ...
    try {
        ...
        const posts = await dbAll(
            `SELECT p.*, u.name, u."profilePic", u."isVerified", u."hasFarmerBadge",
             ...
             FROM posts p
             JOIN users u ON p."userId" = u.id
             WHERE p."isActive" = TRUE AND LOWER(p.content) LIKE $2 ESCAPE '\\'
             ORDER BY p."createdAt" DESC
             LIMIT $3 OFFSET $4`,
            [req.user.id, searchTerm, parseInt(limit), offset]
        );
    ...
    try {
        ...
        const users = await dbAll(
            `SELECT u.id, u.username, u.name, u."profilePic", u.bio, u."isVerified",
             ...
             FROM users u
             WHERE (u.username ILIKE $2 ESCAPE '\\' OR u.name ILIKE $2 ESCAPE '\\') AND u.id != $1 AND u."isActive" = TRUE
             ORDER BY u."isVerified" DESC,
                CASE WHEN u.username ILIKE $3 ESCAPE '\\' THEN 1 WHEN u.name ILIKE $3 ESCAPE '\\' THEN 2 ELSE 3 END,
                (SELECT COUNT(*) FROM follows WHERE "followingId" = u.id) DESC
             LIMIT $4 OFFSET $5`,
            [req.user.id, searchTerm, `${escaped}%`, limitNum, offset]
        );

        const totalResult = await dbGet(
            `SELECT COUNT(*) as count FROM users u WHERE (u.username ILIKE $1 ESCAPE '\\' OR u.name ILIKE $1 ESCAPE '\\') AND u.id != $2 AND u."isActive" = TRUE`,
...
app.post('/api/auth/verify-2fa', validateAuthInput, async (req, res) => {
    try {
        ...
        if (userWithoutPassword.email) userWithoutPassword.email = maskEmail(userWithoutPassword.email);
...
app.post('/api/auth/not-me', async (req, res) => {
    try {
        ...
        const user = await dbGet(
            `SELECT id, name, email, username, "isActive", "isBanned", "twoFactorEnabled", role
             FROM users WHERE (email = $1 OR username = $1) AND "isActive" = TRUE`,
            [loginId]
        );
        if (!user) return res.status(404).json({ error: 'Kullanıcı bulunamadı' });
    ...
    try {
        ...
        const hashtags = await dbAll(
            `SELECT tag, COUNT(*) as count FROM (
             ...
             ) t WHERE tag ILIKE $1 ESCAPE '\\' GROUP BY tag ORDER BY count DESC LIMIT 20`,
            [`${escapeLike(q)}%`]
        );
        res.json({ hashtags });
    } catch (error) {
    ...
    try {
        ...
        const posts = await dbAll(
            `SELECT p.*, u."profilePic" as "userProfilePic", u.name as "userName", u."isVerified" as "userVerified",
                ...
                EXISTS(SELECT 1 FROM saves WHERE "postId" = p.id AND "userId" = $1) as "isSaved"
             FROM posts p JOIN users u ON p."userId" = u.id
             WHERE p."isActive" = TRUE AND u."isActive" = TRUE AND p.content ILIKE $2 ESCAPE '\\'
             ORDER BY p."createdAt" DESC LIMIT $3 OFFSET $4`,
            [req.user.id, `%#${escapeLike(tag)}%`, limitNum, offset]
        );
...
app.get('/api/products', authenticateToken, async (req, res) => {
    try {
        ...
        if (q){ conds.push(`(name ILIKE $${pi} ESCAPE '\\' OR description ILIKE $${pi} ESCAPE '\\')`); params.push(`%${escapeLike(q)}%`); pi++; }
        const products = await dbAll(
...
// 🔒 Unsubscribe token üretici (e-posta gönderirken kullanılır)
...
app.get('/api/email/unsubscribe/:userId', async (req, res) => {
    try {
        ...
        const { token } = req.query;
        // 🔒 IDOR düzeltmesi: HMAC token olmadan herhangi bir userId'nin bildirimlerini kapatmak mümkündü
        if (!token || !verifyUnsubscribeToken(userId, token)) {
            return res.status(403).send('<html><body style="font-family:sans-serif;text-align:center;padding:40px"><h2>❌ Geçersiz bağlantı.</h2><p>Bu bağlantı geçersiz veya süresi dolmuş.</p></body></html>');
        }
        await dbRun('UPDATE users SET "emailNotifications"=FALSE,"updatedAt"=NOW() WHERE id=$1', [userId]).catch(()=>{});
...
app.get('/api/email/resubscribe/:userId', async (req, res) => {
    try {
        ...
        await dbRun('UPDATE users SET "emailNotifications"=TRUE,"updatedAt"=NOW() WHERE id=$1', [userId]).catch(()=>{});
...
app.post('/api/auth/reset-password-with-token', async (req, res) => {
    try {
        ...
        const cleanUsername = username.toLowerCase().trim();
        const user = await dbGet(
            `SELECT id, username, email, name, "isActive" FROM users WHERE LOWER(username) = $1 AND "isActive" = TRUE`,
            [cleanUsername]
        );
        if (!user) return res.status(404).json({ error: 'Kullanıcı bulunamadı' });
...
// 🔒 Rate limit: token brute force saldırısını engelle
...
app.get('/api/verification/action/:token/:action', verificationActionLimiter, async (req, res) => {
    const { token, action } = req.params;
    if (!['approve', 'reject'].includes(action)) return res.status(400).send('Geçersiz işlem.');

    // 🔒 Token format kontrolü (hex 64 karakter)
    if (!/^[a-f0-9]{64}$/.test(token)) return res.status(400).send(buildActionResultPage(false, 'Geçersiz token formatı.'));
    ...
    try {
        ...
        // 🔔 FCM Push bildirimi — onay veya red
        const userId = vReq['userId'] || vReq.userId;
        setImmediate(async () => {
            try {
                if (approved) {
                    // In-app bildirim
                    await createNotification(userId, 'verification_approved',
                        '🏅 Kimlik doğrulamanız onaylandı! Mavi tik hesabınıza eklendi.',
                        { type: 'verification_approved', userName }
                    ).catch(() => {});
                    // FCM push
                    await sendFcmPush(userId, {
                        title: '✅ Doğrulama Onaylandı',
                        body : 'Tebrikler! Kimlik doğrulamanız kabul edildi. Hesabınıza mavi tik eklendi. 🏅',
                        data : { type: 'verification_approved', screen: 'Profile' },
                    }).catch(() => {});
                } else {
                    // In-app bildirim
                    await createNotification(userId, 'verification_rejected',
                        '❌ Kimlik doğrulama başvurunuz reddedildi. Belgeleri kontrol edip tekrar başvurabilirsiniz.',
                        { type: 'verification_rejected', userName }
                    ).catch(() => {});
                    // FCM push
                    await sendFcmPush(userId, {
                        title: '❌ Doğrulama Reddedildi',
                        body : 'Kimlik doğrulama başvurunuz reddedildi. Belgelerinizi kontrol edip tekrar başvurabilirsiniz.',
                        data : { type: 'verification_rejected', screen: 'VerificationRequest' },
                    }).catch(() => {});
                }
            } catch (e) {
                console.error('[Verification FCM hatası]', e.message);
            }
        });
