...
function generateTokens(user) {
    ...
    const accessToken = jwt.sign(
        ...
        JWT_SECRET,
        { expiresIn: '2h', algorithm: 'HS256' } // 🔒 30d → 2h: güvenlik + kullanılabilirlik dengesi
    );
...
function setAuthCookies(res, req, tokens) {
    ...
    res.cookie('access_token', tokens.accessToken, {
        ...
        sameSite : 'strict',
        path     : '/',
        maxAge   : 2 * 60 * 60 * 1000, // 🔒 2 saat — JWT expiry ile eşleştirildi
    });
...
function setCsrfCookie(res, req, token) {
    ...
    res.cookie('csrf_token', token, {
        ...
        sameSite : 'strict',
        path     : '/',
        maxAge   : 2 * 60 * 60 * 1000, // 🔒 2 saat (access token ile eşleşti)
    });
}
